    ...
    <Link to={`/jobs/${job.job_id}/${job.service === "vision" ? "rollup" : job.service}`}>
      <Card className="hover:bg-accent/50 transition-colors cursor-pointer h-full">
        ...
        <CardContent className="space-y-3">
          ...
          {isActive && <JobProgress progress={job.progress} />}

          {/* Result summary for completed jobs */}
          {job.status === "completed" && job.result_summary && (
            <div className="flex gap-2 text-xs">
              {(job.result_summary as Record<string, number>).faces_count !==
                undefined && (
              ...
              {(job.result_summary as Record<string, number>).scenes_count !==
                undefined && (
                <Badge variant="outline">
                  {(job.result_summary as Record<string, number>).scenes_count}{" "}
                  scenes
